...
// --- Z-AXIS CONTROL (SAFE PLUNGER LOGIC) ---
void moveZ(bool down) {
  ...
  if (down) {
    ...
    // SAFE PLUNGE: Step cap prevents infinite runaway if switch fails
    long plungeSteps = 0;
    const long MAX_PLUNGE_STEPS = 4000; 
    
    while (digitalRead(Z_STOP_PIN) == Z_OPEN) {
      stepZ(1200); 
      plungeSteps++;
      if (plungeSteps > MAX_PLUNGE_STEPS) {
        Serial.println("ERR:Z_LIMIT_TIMEOUT");
        break; // Hard cut-off to save mechanical hardware
      }
    }
...
void loop() {
  if (Serial.available() > 0) {
    ...
    // NEW: Sensor poll command (safely test limit switches without moving motors)
    if (cmd == 'S' || cmd == 's') {
      int zState = digitalRead(Z_STOP_PIN);
      int lState = digitalRead(L_LIMIT_PIN);
      int rState = digitalRead(R_LIMIT_PIN);
      
      Serial.print("STATUS: Z_PIN10=");
      Serial.print(zState);
      Serial.print(" L_PIN9=");
      Serial.print(lState);
      Serial.print(" R_PIN13=");
